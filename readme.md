# Available documentation:

Hello there! If you're here, that means you have write access and can add/modify any information that's available.

## Development

- [Getting Started with Plugins](Plugins/getting-started.md), by Darkhax, Jared & Rick
- [Getting Started with Packs](Packs/readme.md), by Sketch Macaw
- [Getting Started with EarlyPlugins & ClassTransformers](https://github.com/Frontier-Modding/Frontier-Modding-Docs/blob/main/Plugins/early-plugins.md) by Darkhax
- [Getting Started with Hyxins (Mixin)](https://github.com/Frontier-Modding/Frontier-Modding-Docs/blob/main/Plugins/hyxin-guide.md) by Darkhax
- [Logging and You](https://github.com/Frontier-Modding/Frontier-Modding-Docs/blob/main/Plugins/Logging.md) by Darkhax

## Config

- [Custom Config Files](Config/Getting-Started.md)

## Data

- [Custom Asset Stores](Data/data.md)
- [Universe Storage](Data/resources.md)

# Entities

- [Entities, Components, and Systems](Entities/entities.md)

## Events:

- [List of Available Events](Events/Available-Events.md), by Rick, updated by Darkhax (pre-19, release-31)

## GUI

- [In-Game Title Message](GUI/titles.md), by tr7zw

## Interactions

- [Item and Entity Interactions](Interactions/interactions.md)

## Map Markers

- [Automated Campfire Map Markers](MapMarkers/AutomatedCampfireMapMarkers.md), by tr7zw

## Sounds:

- [Playing Sounds](Sounds/playing-sounds.md), by tr7zw

# Misc

- [wiki.vg](wiki.vg/index.md), by Up
